#include "Snake.h"
#include <iostream>
Snake::Snake(float l_blockSize, Textbox* l_log)
{
	m_log = l_log;
	m_size = l_blockSize;
...
{
	m_log->AdditoinalBox("Speed Increased!");
	++m_speed;
...
void Snake::DecreaseSpeed()
{
	...
	Extend();
	--m_speed;
	m_log->AdditoinalBox("Speed Decreased!");
}
...
{
	m_log->Add("Score: " + std::to_string((unsigned long)m_score));
	m_log->Add("Speed: " + std::to_string((unsigned long)m_speed));
}	
void Snake::Lose()
{ 
	m_snakeBody.clear();
...
void Snake::Reset()
{
	...
	SetDirection(Direction::None); // Snake standing still
	m_speed = 15;
	m_lives = 3;
	m_score = 0;
	m_lost = false;
...
void Snake::Cut(int l_segments) 
{
	...
	m_log->AdditoinalBox("You ate yourself!!      Lives left: " + std::to_string((unsigned long)m_lives));
}
...
void Snake::Render(sf::RenderWindow& l_window) 
{
	...
	m_bodyRect.setFillColor(sf::Color::Yellow); //Head Color
	m_bodyRect.setPosition(head->position.x * m_size, head->position.y * m_size);
	l_window.draw(m_bodyRect);


	m_bodyRect.setFillColor(sf::Color::Green); //Body Color
	for (auto itr = m_snakeBody.begin() + 1;itr != m_snakeBody.end(); ++itr)
	{
